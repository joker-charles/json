...
- 访问策略：反射默认用 `access_context::unprivileged()`（`codec<false>`）——
  ...
  （§2.3 的两层策略，§2.5 中有演示）。
- 一次性成本：头文件共 **423 行** —— 278 行代码（含 10 行 `#include`/`#pragma once`）、
  101 行注释（其中 69 行是设计/陷阱/覆盖边界文档块）、44 行空行。这取代了早先内联的
  `namespace refl2` 块（311 行 + 1 行 `#include <meta>` = 312 行）；增加的部分换来
  下面要讲的优化机制、`std::optional` 防护与已文档化的覆盖边界。
...
带可操作信息；下面每一项扩展都会把一次性成本抬到当前 423 行之上）：
...
**`std::optional<T>` 受支持**，经 adl 分支原生处理——序列化为 `T` / `null`，
与 nlohmann 自己的 optional 支持一致。这需要一个显式的 `is_optional` 排除：
C++23 给 `std::optional` 加了 `begin()`/`end()` + `value_type`，否则它会被误判为
array-like，把 `optional<int>{5}` 静默序列化成 `"[5]"` 而不是 `"5"`（修复前已实测；
探针 [D] 见 §2.5）。`optional<PlainStruct>`（T 不可构造进 nlohmann）落到干净的
优先级 0 static_assert。
...
- **场景 A —— 用户自己编写序列化器**（本评估的现状：编解码器是一个用户复制进
  ...
  私有成员策略 + 已文档化的覆盖边界）需要**423 行**一次性序列化器
  （盈亏平衡 ≈ 423 个类型），能处理嵌套结构体、普通结构体容器、用户定制、
  `std::optional` 与 `unprivileged()` 访问控制——这些 v1 完全做不到。
...
它 include 共享的 `refl2_codec.hpp`（不再内联副本），运行 **24 项检查，全部 PASS，
-O0 与 -O1 下 ASan 干净**：
...
- **[D] `std::optional`** —— C++23 `begin`/`end` + `value_type` 防护：
  `optional<int>` 被分类为 adl 可用（绝不视为数组），序列化结果与原生 nlohmann
  逐字节一致（`5` / `null`，往返已验证）；而 `optional<Address>`（普通结构体）
  被分类为不可 adl / 不可数组——它命中干净的优先级 0 static_assert。
...
- §2.5 探针：**24 项检查全部 PASS，-O0 与 -O1 下 ASan 干净**——含 C 数组分类检查
  与 [D] `std::optional` 防护检查。
