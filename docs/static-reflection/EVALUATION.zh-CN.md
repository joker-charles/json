...
- **反射每类型省 1 行**，且结构性杜绝静默遗漏（宏的 omission 危害不可能
  ...
  v2 = 完整 ADL 感知递归编解码器 "refl2"（667 行整个文件，纯代码 463），
  盈亏平衡 ≈ 25 / ≈ 667（≈ 463）类型。（定义见 §2.1。）
...
- 访问策略：反射默认用 `access_context::unprivileged()`（`codec<false>`）——
  ...
  （§2.3 的两层策略，§2.5 中有演示）。
- 一次性成本：头文件共 **667 行** —— 463 行代码（含 10 行 `#include`/`#pragma once`）、
  148 行注释（其中 89 行是设计/陷阱/覆盖边界文档块）、56 行空行。这取代了早先内联的
...
带可操作信息；下面每一项扩展都会把一次性成本抬到当前 667 行之上）：
...
- **场景 A —— 用户自己编写序列化器**（本评估的现状：编解码器是一个用户复制进
  ...
  v2 = 667 行（refl2_codec.hpp 整个文件；纯代码体 463 行——148 注释 / 56 空行
  是设计/陷阱/覆盖边界文档，见 §2.1；只算代码时 v2 盈亏平衡移到 ≈ 463 类型）。
  ...
  **667 行**一次性序列化器（盈亏平衡 ≈ 667 个类型），能处理嵌套结构体、
...
- **扩展路径测量（继承 / optional / 位域）**（本修订）：`bench_extended.cpp`
  ...
  - 运行时：deserialize 一致更快（继承 −22%、optional −20%、位域 −37%）、
    ...
    典型宏）。
  - Q1 场景 A 的盈亏平衡给出双口径：667 行（整个文件，保守）与 463 行
    （纯代码体；≈ 463 类型）。
