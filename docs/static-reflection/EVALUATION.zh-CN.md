...
- 访问策略：反射默认用 `access_context::unprivileged()`（`codec<false>`）——
  ...
  （§2.3 的两层策略，§2.5 中有演示）。
- 一次性成本：头文件共 **565 行** —— 386 行代码（含 10 行 `#include`/`#pragma once`）、
  129 行注释（其中 83 行是设计/陷阱/覆盖边界文档块）、50 行空行。这取代了早先内联的
  `namespace refl2` 块（311 行 + 1 行 `#include <meta>` = 312 行）；增加的部分换来
  下面要讲的优化机制、`std::optional` 分支、继承（subobjects_of）递归与已文档化的
  覆盖边界。
...
两种形态是编译错误而非静默丢失：**私有/受保护基类**在 `codec<false>` 下
（`has_inaccessible_bases`；改用 `codec<true>` 或加 `to_json`）与**跨层级重名成员**
（基类与派生同名，或菱形继承）——两者都会静默覆盖 JSON 键。
...
带可操作信息；下面每一项扩展都会把一次性成本抬到当前 565 行之上）：
...
- **场景 A —— 用户自己编写序列化器**（本评估的现状：编解码器是一个用户复制进
  ...
  私有成员策略 + 继承 + `std::optional` + 已文档化的覆盖边界）需要**565 行**
  一次性序列化器（盈亏平衡 ≈ 565 个类型），能处理嵌套结构体、普通结构体容器、
  用户定制、基类成员与 `unprivileged()` 访问控制——这些 v1 完全做不到。
...
它 include 共享的 `refl2_codec.hpp`（不再内联副本），运行 **33 项检查，全部 PASS，
-O0 与 -O1 下 ASan 干净**：
...
- **[F] 继承** —— 基类成员经 `subobjects_of` 递归序列化：多层的
  ...
  trait 化检查：unprivileged 策略下检测私有基类（`has_inaccessible_bases`）、
  检测菱形层级里的重名成员；`codec<true>` 连私有基类成员一起序列化。
...
- §2.5 探针：**33 项检查全部 PASS，-O0 与 -O1 下 ASan 干净**——含 C 数组分类检查
  与 [D] `std::optional` / [F] 继承检查。
...
- §2.1 覆盖边界，**继承**（本修订）：边界条目（"基类成员静默忽略，需要 bases_of
  ...
  423 → 565 行，Q1 已重算（盈亏平衡 ≈ 565 类型）；探针 33 项。基准扫描数字不受
  影响（基准类型为扁平结构，无 optional/继承）——N=50 -O2 宏 vs refl2 用新 codec
  复核尺寸一致（各 182,608 B）。
