// bench_complex.cpp — compile-time / binary-size benchmark on COMPLEX structs
// (vs bench_macro_vs_reflection.cpp, which uses a flat 5-field person). The
// complexity here exercises the three sources of the refl2 per-type cost:
// nested-member recursion (Leaf), container element recursion
// (vector<Leaf> / map<string,Leaf> / set<int> / vector<vector<int>>), and
// multi-level base flattening (Mid : Base). Two modes, same TU, same standard
// and flags — only the serializer source differs, selected by the -D flag:
//
//   macro mode:   g++-16 -std=c++26 -freflection -O2 -DBENCH_N=50 -Iinclude \
//                   -o /tmp/bench_complex_macro bench_complex.cpp
//   refl2 (v2):   g++-16 -std=c++26 -freflection -O2 -DBENCH_N=50 \
//                   -DBENCH_ADL_REFLECTION -Iinclude \
//                   -o /tmp/bench_complex_refl2 bench_complex.cpp
//
// (The v1 naive serializer is intentionally absent: it is flat-struct-only
// and cannot serialize the nested/container/inherited members here.)
//
//   macro mode  = NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE (Leaf/Base/Mid) +
//                 NLOHMANN_DEFINE_TYPE_INTRUSIVE (person, in-class)
//   refl2 (v2)  = the ADL-aware recursive codec (zero boilerplate).
//
// BENCH_N (default 50) = how many person structs are actually instantiated
// in main(); all 100 definitions are always present, but unused types
// generate no code, so -DBENCH_N sweeps the marginal cost.
//
// complex person: `struct person : Mid { Leaf leaf; vector<Leaf> leaves;
//   map<string,Leaf> by_name; set<int> ids; vector<vector<int>> matrix;
//   int id; string name; bool flag; }` (Mid flattens Base's seq/tag + ratio/hist).
//
// Measure with:
...
#include <nlohmann/json.hpp>
    ...
    #include <meta> // v1 serializer; refl2_codec.hpp (BENCH_ADL_REFLECTION) includes it itself
#endif
...
// ---------------------------------------------------------------------------
// The ADL-aware recursive codec (BENCH_ADL_REFLECTION mode). Lives in
// refl2_codec.hpp (single source of truth shared with probe_adl_recursion.cpp
...
// ---------------------------------------------------------------------------
// 100 complex structs (nested leaf + containers + inheritance), generated by
// the PERSON() X-macro. The complexity here (vs the simple 5-field person in
// bench_macro_vs_reflection.cpp) exercises nested-member recursion, container
// element recursion, and multi-level base flattening — the sources of the
// refl2 per-type compile cost. In macro mode each also gets
// NLOHMANN_DEFINE_TYPE_INTRUSIVE; in reflection mode the plain struct is all
// the user writes.
// ---------------------------------------------------------------------------
...
#ifdef BENCH_REFLECTION
#define DEFINE_PERSON(i) struct person_##i : Mid \
    { \
        PERSON_FIELDS \
    };
#else
...
#define DEFINE_PERSON(i) struct person_##i : Mid \
    { \
        NLOHMANN_DEFINE_TYPE_INTRUSIVE(person_##i, seq, tag, ratio, hist, id, name, leaf, leaves, by_name, ids, matrix, flag) \
        PERSON_FIELDS \
    };
#endif
